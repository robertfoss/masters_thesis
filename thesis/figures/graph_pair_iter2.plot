...
%	\node[op] (n9)  at (12, 2)	{\%9 mul};
	...
	\node[op] (n10) at (6, 0)	{\%10--\%11 add};
%	\node[op] (n11) at (10, 0)	{\%11 add};
	...
	\path[->] (n8) edge [bend right =-15] node[below =0.15 cm] {} (n10);
	\path[->] (n8) edge [bend right = 45] node[below =0.15 cm] {} (n10);
	\path[->] (n8) edge [bend right =-45] node[below =0.15 cm] {} (n10);
	
\end{tikzpicture}
\end{adjustbox}
\end{center}
\caption[Operation graph after two iterations of pair based vectorization]{Graph of operations that have been vectorized two itertions with the pair based vectorizer.}
\end{figure}